...
test('uses http for local targets', () => {
    assert.equal(getURLTarget('localhost:9'), 'http://localhost:9');
...
test('rejects invalid or unsafe URL-like input', () => {
    ...
    assert.equal(isValidURL('999.999.999.999:9'), false);
    assert.equal(isValidURL('https://example.com:99999'), false);
    assert.equal(isValidURL('localhost:99999'), false);
